 ...
 *
 */
public class OneColumnDialog extends Stage implements Encapsulation, Utilities
{
    /*          Properties   	                                                                                      */
    /******************************************************************************************************************/
    protected Scene		scene;			// Dialog element scene
    protected Stage	 	stage;			// Form which created this dialog
    ...
    protected AssociativeList	attributesList;         // Form atttributes List
    ...
    */
     ...
     * @param elList Associative list to store dialog elements list
     * @return Filled combo box
     */
    protected ComboBox fillComboBox( DialogElement el, AssociativeList	elList )
    {
        String[] textChoices = null;  // To store text choices for combo box

        // If there are choices specified for the current element
        if ( el.textChoices.length > 0 )
            textChoices = el.textChoices;
        ...
        else if ( el.list != null)
        {
            ...
            /*
            // Loop for each list element
            for ( int p = 0; p < el.list.size(); p++ )
            {
                // Get list item
                itm = (Item) el.list.get( p );
        ...
        // If default choice is specified
        if ( el.defaultChoice != -1 && textChoices.length > 0 )
        {
            ...
            if ( el.onChange != null )
                ...
                el.onChange.run( textChoices[el.defaultChoice], elList ); 
        }
        
        // If there is the only choice in the list of choices
        else if ( textChoices.length == 1 )
        {
            // Set this the only choice as current combo box value
            comboBox.setValue( textChoices[0] );

            // If action on dialog element change is specified
            if ( el.onChange != null )
                // Invoke specified lambda expression with parameters
                el.onChange.run( textChoices[0], elList );
        }
        ...
        if ( el.onChange != null )
        {
            ...
            comboBox.valueProperty().addListener( new ChangeListener() 
            {
                ...
                {
                   // Invoke specified lambda expression with parameters
                   el.onChange.run( newValue, elList ); 
                }
            } );
        }
    ...
    protected void createComboBox( GridPane grid, DialogElement el, int col, int row )
    {
        ...
        ComboBox comboBox = fillComboBox( el, attributesList );
    ...
    protected void createTextField( GridPane grid, DialogElement el, int col, int row )
    {
        ...
        if ( el.width > 0 )
        {
           ...
           // Set number field width
            ...
            // Set column width for text field
        ...
        if ( el.textValue != null && !el.textValue.isEmpty() )
            ...
            //    textField.setText( formatter.format( Double.parseDouble( el.textValue ) ) );
            //}
            //else
                // Set text field to specified text value
                textField.setText( el.textValue );
    ...
    public OneColumnDialog( RegistryItem doc, String title )
    {
        ...
        this.stage = (Stage) attr.get( "stage" );
        this.tabs = (String[]) attr.get( "headerTabs" );
        this.dialogElement = (DialogElement[][]) attr.get( "header" );
        
        // Create associative list for attributes list
        attributesList = attr;
    ...
    {
        this( stage, title, null, new DialogElement[][]{dlg} );
    }
