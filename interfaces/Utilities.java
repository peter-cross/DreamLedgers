...
public interface Utilities extends Encapsulation
{
    ...
    public static RegistryItem getListElementBy( HashSet list, String field, String value )
    {
        ...
        while ( it.hasNext() )
        {
            ...
            if ( ((String) fields.get(field)).trim() == value )
                return elm;
        }
    ...
    default String getFieldNumber( String fieldname )
    {
        ...
        if ( obj == null )
            ...
            try
            {
                // If object is convertable to double value
                if ( (double) obj > 0 )
            ...
            {
                // If object is convertable to int
                if ( (int) obj > 0 )
    ...
    static Class tryCreateDataClass( String pckg, String valueType )
    {
        ...
        if ( pckg.isEmpty() )
            ...
            {
                // Try invokemethod again for package data
            ...
            catch ( Exception e )
            {
                try
                {
                    // Try invokemethod again for subpackage references
            ...
            switch ( field )
            {
                case "G/L Number":
                    if ( !value.isEmpty() )
                        try
                        {
                            // Try to parse input value into double number
                        ...
                        {
                            // Validation message if could nor parse the value
                            return validationMessage( "The '" + field + "' field should be a valid number " );
                        }
                default:
